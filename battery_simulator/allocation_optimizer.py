...
def allocate_bid_ask_with_cost(ask_dict, bid_dict, init_state, end_state, alpha, action_pair_cost=10):
    n = len(ask_dict)
    ...
    objective = cp.Maximize(alpha * (bid_array @ x_bid - ask_array @ x_ask - cp.sum(x_ask +x_bid)*action_pair_cost/2) - (1 - alpha) * cp.sum(x_ask + x_bid))  # spend/buy on the ask, earn/sell on the bid
    ...
    num_actions = np.sum(x_ask.value + x_bid.value, dtype=int)/2
...
def allocate_bid_ask_new_with_cost(ask, bid, old_ask, old_bid, old_alloc, init_state, end_state, alpha, action_pair_cost=10):
    ...
    objective = cp.Maximize(alpha * (bid_array @ x_bid - ask_array @ x_ask +  # nominal profit
                                     ...
                                     (old_bid_array - bid_array) @ sold_before_sold_now -
                                     (cp.sum(x_ask+x_bid)*action_pair_cost/2)) + (1 - alpha) * cp.sum(x_ask + x_bid))  # -1 -> -1
    ...
    # extract the values and returns the profit, deducting the cost per number of action
    max_profit = objective.value
    x_opt = x_ask.value - x_bid.value
    num_actions = np.sum(x_ask.value +x_bid.value, dtype=int)/2
...
def get_initial_allocation(ask_dict, bid_dict, init_state=4, end_state=4, alpha=1, action_pair_cost=10):
    old_ask = ask_dict
    old_bid = bid_dict

    initial_allocation, initial_profit, number_of_actions = allocate_bid_ask_with_cost(old_ask, old_bid, init_state, end_state, 1, action_pair_cost)

    return initial_allocation, np.round(initial_profit,2), number_of_actions


def get_new_allocation(ask, bid, old_ask, old_bid, old_allocation, init_state, end_state, alpha=1, action_pair_cost=10):
    old_ask, old_bid, old_allocation = trim_old_allocation(old_ask, old_bid, ask, old_allocation)  # remove the keys that have expired
    new_allocation, profit, number_of_actions = allocate_bid_ask_new_with_cost(ask, bid, old_ask, old_bid, old_allocation, init_state, end_state, alpha, action_pair_cost=action_pair_cost)

    # profit  = allocation_profit + cost_profit (cost_profit = trading_part)
...
def get_initial_allocation2(ask_dict, bid_dict, init_state=4, end_state=4, alpha=1, number_of_action_cost=10):
    old_ask = ask_dict
    old_bid = bid_dict
    ...
    # for 3 mw. 6eur normally for 1 mw buysell pair assuming 2.5million investment and 10mw battery
...
def get_initial_allocation2_threaded(ask_dict, bid_dict, init_state=4, end_state=4, alpha=1):
    old_ask = ask_dict
    old_bid = bid_dict
    ...
    def get_allocation(l):
        new_allocation, profit, number_of_actions = allocate_bid_ask(old_ask, old_bid, init_state, end_state, alpha[l])
        return new_allocation, profit, number_of_actions

    if __name__ == 'allocation_optimizer':
        processes = [Process(target=get_allocation, args=(i,)) for i in range(0, n_alpha)]
    ...
    for i, future in enumerate(processes):
        new_allocation, profit, number_of_actions = future.result()
        if number_of_actions > 0:
            if (profit - cost_factor_for1mwtrade * number_of_actions) / number_of_actions > best_fraction:
                best_fraction = profit / number_of_actions
                info_of_best = [new_allocation, profit, number_of_actions]
...
def get_new_allocation2(ask, bid, old_ask, old_bid, old_allocation, init_state, end_state, alpha=1, number_of_action_cost=10):
    old_ask, old_bid, old_allocation = trim_old_allocation(old_ask, old_bid, ask, old_allocation)  # remove the keys that have expired
    ...
    results = results.drop_duplicates(subset=['number_of_actions'], keep='last')
    results['profit_diff'] = results['profit'].diff()
    best_result = results.iloc[[-1], :]
    try:
        for i in range(len(results)-1, -1, -1):
            if results.profit_diff.iloc[i] < number_of_action_cost:
                continue
            else:
                best_result = results.iloc[[i], :]
                break
    except:
        print('error on choosing best_result from best_allocations given number of actions ')
        pass
...
def get_true_current_bid_ask(old_ask, old_bid, ask, bid, old_allocation, new_allocation):
    """
    You will see this function is used to get the self.trueoldask and self.trueoldbid in the main runner.
    Only the values for keys , where position is open. If we hold the same position in the new allocation, we update carry on the old values, since we didnt make any moves
    We have to compare with the ask and bids when we made a trade.
    * bit messy implementation, since we also parse the keys where we dont have positions.
    """
    i = 0
